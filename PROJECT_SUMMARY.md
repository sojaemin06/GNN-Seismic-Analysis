...
### 2.2. 구조 모델링 및 데이터 검증
- **재료 강도:** '내진성능 평가요령'에 따라 공칭강도가 아닌 **기대강도**를 사용하여 부재 강도를 산정한다. (`src/core/model_builder.py`)
...
### 목표 1: 학습 데이터셋 생성
- **내용:** `scripts/generate_dataset.py` 스크립트를 실행하여, 다양한 구조 및 재료 파라미터를 갖는 RC 모멘트 골조 모델에 대한 대규모 데이터셋을 생성한다.
- **산출물:**
