...
### 2.2. 구조 모델링 및 데이터 검증
- **재료 강도:** '내진성능 평가요령'에 따라 부재의 비선형 거동을 평가하기 위해 공칭강도가 아닌 **기대강도**를 사용한다. 단, 콘크리트의 탄성계수는 공칭강도를 기반으로 한 KBC 기준식을 따른다.
...
- **데이터 유효성 검증:** 생성되는 모든 구조물 데이터는 다음 조건을 만족해야 한다.
    ...
    3.  **재료 변형률 한계:** 콘크리트 압축 변형률 0.003, 철근 인장 변형률 0.05를 초과하는 재료 파괴가 발생하지 않아야 한다.
