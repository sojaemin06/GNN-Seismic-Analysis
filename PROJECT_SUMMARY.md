...
### 2.2. 구조 모델링 및 데이터 검증
- **재료 강도:** '내진성능 평가요령'에 따라 부재의 비선형 거동을 평가하기 위해 공칭강도(`f_ck_nominal`)가 아닌 **기대강도(`fc`, `Fy`)**를 사용한다. 콘크리트의 탄성계수(`Ec`)는 공칭강도(`f_ck_nominal`)를 기반으로 한 KBC 기준식(`Ec = 8500 * ³√f_cu`, 여기서 `f_cu = f_ck_nominal + Δf`)을 따른다.
- **재료 모델 파라미터:**
    - **콘크리트 (`Concrete04`):** Popovics 모델을 기반으로, 인장 강도는 무시하고 비구속 콘크리트의 극한변형률은 0.003, 잔류응력비는 0.1을 적용한다.
    - **철근 (`MultiLinear`):** 연성 거동이 지배적인 인장부는 **기대강도**를 적용하고, 좌굴에 의한 취성 파괴가 우려되는 압축부는 **공칭강도**를 적용하여 보수적으로 모델링한다. 압축 변형률 -0.003에서 좌굴이 시작되고, -0.005까지 강도가 선형 감소하며, 이후 극한 변형률(-0.02)까지 잔류 강도를 유지하는 다중선형 모델을 사용한다.
- **해석 모델링 기법:** OpenSeesPy의 **섬유요소모델(Fiber Element Model)**을 사용하여, 부재의 비선형 휨 거동과 축력-모멘트 상호작용을 정밀하게 모사한다. 단, 이 모델은 **전단파괴나 정착파괴가 아닌 휨 파괴가 구조물의 거동을 지배한다고 가정**하며, 보-기둥 접합부는 강절점으로 단순화하여 모델링한다.
- **요소별 단면 변형 기록:** 재료 파괴 검사를 위해 각 요소(기둥, 보)의 콘크리트 코어 최외곽 4개 지점의 변형률을 `fiber` 레코더를 통해 개별 파일로 기록한다. (`src/core/model_builder.py`)
- **데이터 유효성 검증:** 생성되는 모든 구조물 데이터는 다음 조건을 만족해야 한다.
    1.  **1차 모드 지배 조건:** 각 해석 방향에 대해, 다중모드 스펙트럼 해석(RSA)의 층전단력이 해당 **방향의 질량참여율이 가장 큰 지배 모드(dominant mode)에 대한 단일모드 RSA 층전단력**의 1.3배를 초과하지 않아야 한다. (`src/core/verification.py`)
    2.  **질량 참여율:** 해석 방향의 유효 질량 참여율이 90% 이상이어야 한다.
    3.  **재료 변형률 기반 파괴 검사:** 콘크리트 압축 변형률 0.003, 철근 인장 변형률 0.05를 초과하는 재료 파괴가 발생하지 않아야 한다. (`src/core/post_processor.py`의 `find_first_material_failure` 함수를 통해 검사)
