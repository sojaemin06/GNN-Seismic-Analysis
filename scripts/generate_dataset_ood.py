...
def main_generate_ood_dataset(num_samples: int = 100):
    dataset_config_path = Path(__file__).parent / 'dataset_config_ood.json'
    processed_data_dir = Path(project_root) / 'data' / 'ood'
    processed_data_dir.mkdir(parents=True, exist_ok=True)

    log_file_path = processed_data_dir / 'dataset_generation_log_ood.txt'
    error_log_file_path = processed_data_dir / 'dataset_generation_errors_ood.txt'
    ...
    print(f"Starting OOD data generation from Sample ID: {start_sample_id}")
    print(f"Target: Generate {num_samples} NEW successful OOD samples.")
    ...
    with open(log_file_path, 'a', encoding='utf-8') as log_f, \
        ...
        log_f.write(f"\n--- OOD Dataset Generation Session Started: {pd.Timestamp.now()} ---\n")
        log_f.write(f"Target: {num_samples} successful samples (Starting ID: {start_sample_id}).\n\n")
        
        with tqdm(total=num_samples, desc="Generating OOD Dataset") as pbar:
        ...
        log_f.write(f"\n--- OOD Dataset Generation Session Finished: {pd.Timestamp.now()} ---\n")
...
if __name__ == '__main__':
    main_generate_ood_dataset(num_samples=100)
